...
- **손실함수를 최소화**하기위 위해 **가중치**를 반복적으로 조정하는 **최적화 알고리즘**
  ...
  - 하이퍼 파라미터: 학습률(learning_rate, step_size, $\alpha$)
  - 한계점
    - 기울기 소실
    ...
    - 과진동
      - 학습률이 너무 커서, 모델이 오차(Loss)의 최솟값을 지나쳐 반대편으로 튀어 오르는 현상
      - 심한경우: 최솟값에 수렴하지 못하고, 오차가 더 커지는 발산 발생
      - 해결법
      	- 적절한 초기 학습률 설정
       	- 점진적으로 학습률 낮추기
        - 관성(Momentom)추가 -> Adam
<img src="./images/gradient_descent_learning_rate.svg">
    ...
    - 지역 극솟값(Local Minima)
      - 시작점은 램덤으로 시작할때, 낮은 학습률때문에 지역 극솟값에 갖힌다.
...
- 사용이유: 2D 데이터(이미지)의 특징을 자동 추출 및 학습하여 높은 성능을 발휘하기 위해서
...
### Pooling Layer (풀링 계층)
- **특징맵(Feature Map)의 공간 크기를 줄여 계산량을 감소**시키고, **중요한 특징을 추출**하며, 과적합을 방지하는 역할을 하는 레이어
...
- 다차원 배열 데이터를 1차원 배열로 변환하는 레이어(입력의 **특징맵들을 1자로 펼침**)
- 과정: input(풀링 후 특징맵(Batch,Height,Width,Channels)) -> Flattening(평탄화) -> output((Batch,Height*Width*Channels)):추출된 특징 묶음)


**<해당 정리내용 추가 및 변경 예정 사항>**
1. CNN 이미지 패딩 기능 추가해서 변경예정
2. 교재의 예제코드 실행 후 전체적인 실행예제코드 코랩에서 직접 싫행한 링크 추가 예정 
